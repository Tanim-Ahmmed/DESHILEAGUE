...
const HomeScreen = () => {
  ...
  const handleNavigateToProfile = () => {
    router.push('/managers/profile' as any);
  };
  ...
  const renderManagerCard = (manager : any, index: number) => (
    <View key={manager.id} style={styles.organizerCard}>
      <Image source={{ uri: manager.image }} style={styles.organizerImage} />
      <Text style={styles.organizerName}>{manager.name}</Text>
      <Text style={styles.teamTitle}>{manager.location}</Text>
    </View>
  );

  const renderTeamRankCard = (team : any, index: number) => (
    <View key={team.id} style={styles.organizerCard}>
      ...
      <Text style={[styles.organizerTitle, {color: "#EAB50F"}]}>Rank - {team.rank}</Text>
    </View>
  );
  ...
  const renderTeamCard = (team : any, index: number) => (
    <View key={team.id} style={styles.organizerCard}>
      <Image source={{ uri: team.image }} style={styles.organizerImage} />
      <Text style={styles.organizerName}>{team.name}</Text>
      <Text style={[styles.organizerTitle,{color: "#EAB50F"}]}>win - {team.totalWins}</Text>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.section}>
          <Text style={[styles.sectionTitle,{color: theme.primaryTextColor}]}>Suggested Tournaments For You</Text>
          <TournamentSlider 
            data={sliderData}
            autoScroll={true}
            autoScrollInterval={5000}
            dynamicRoute="managers/tournament-list/details/"
          />
        </View>

        <View style={styles.createButtonContainer}>
            <TouchableOpacity style={styles.createButton} onPress={() => router.push('/managers/my-teams/create-team' as any)}>
        ...
        <View style={[styles.section, {borderBottomWidth: 2, borderBottomColor: theme.cardBorder , marginHorizontal: 12, paddingHorizontal: 0 , paddingBottom : 12}]}>
          <Text style={[styles.sectionTitle,{color: theme.primaryTextColor}]}>My Teams Rank</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
            <View style={styles.cardsContainer}>
              {topTeamsRankData.map((team, index) => renderTeamRankCard(team, index))}
            </View>
          </ScrollView>
        </View>
        <View style={styles.section}>
          <Text style={[styles.sectionTitle,{color: theme.primaryTextColor}]}>Top Manager</Text>
          <ScrollView horizontal showsHorizontalScrollIndicator={false} style={styles.horizontalScroll}>
            <View style={styles.cardsContainer}>
              {topManagerData.map((manager, index) => renderManagerCard(manager, index))}
            </View>
          </ScrollView>
        </View>
        <View style={[styles.section]}>
